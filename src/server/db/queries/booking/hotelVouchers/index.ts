...
export const updateHotelVoucherRate = async (data: SelectHotelVoucherLine) => {
    try {
      await db.update(hotelVoucherLine)
        .set({ rate: data.rate })
  ...
  export const bulkUpdateHotelVoucherRates = async (voucherLines: SelectHotelVoucherLine[]) => {
    try {
      await db.transaction(async (trx) => {
        // Create an array of update operations
        for (const voucherLine of voucherLines) {
          await trx.update(hotelVoucherLine)
