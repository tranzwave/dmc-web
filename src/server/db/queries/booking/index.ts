...
import { booking, bookingLine, hotelVoucher, hotelVoucherLine, client } from "../../schema";
import { BookingDetails, HotelVoucher } from "~/app/dashboard/bookings/add/context";
...
export const createNewBooking = async (
  ...
  try {
    ...
    if (!parentBooking) {
      parentBooking = await db.insert(booking).values({
      ...
      if (!parentBooking || !Array.isArray(parentBooking) || !parentBooking[0]?.id) {
        throw new Error("Couldn't add new parent booking");
      }
    }

    // Now we know parentBooking is an array with at least one item
    const parentBookingIdToUse = Array.isArray(parentBooking) ? parentBooking[0]?.id || "": parentBooking.id;
    ...
    if (bookingDetails.general.includes.hotels) {
      await insertHotelVouchers(bookingDetails.vouchers, lineId, bookingDetails.general.marketingManager);
    }
...
export const createBookingLine = async (data: InsertBookingLine): Promise<string> => {
...
export const insertHotelVouchers = async (vouchers: HotelVoucher[], newBookingLineId: string, coordinatorId:string) => {
  const hotelVouchers = await Promise.all(
    vouchers.map(async (currentVoucher) => {

      // Add a voucher
      const newVoucher = await db.insert(hotelVoucher).values({
      ...
      const voucherLines = await Promise.all(
        currentVoucher.voucherLines.map(async (currentVoucherLine) => {
          const newVoucherLine = await db.insert(hotelVoucherLine).values({
