...
export const getAllRestaurants = async () => {
  try {
    const orgId = await getActiveOrganization().then((orgId) => orgId);
    return db.query.restaurant.findMany({
      where: eq(restaurant.tenantId, orgId),
      with: {
        restaurantMeal: true,
      },
    });
...
export const getCoordinatorAndManager = (
  cooridnatorId: string,
  managerId: string,
) => {
...
export const updateRestaurantVoucherRate = async (
  data: SelectRestaurantVoucherLine,
) => {
  try {
    await db
  ...
  try {
    await db.transaction(async (trx) => {
      ...
      for (const [lineId, rate] of ratesMap) {
        const existingVoucherLine =
          await trx.query.restaurantVoucherLine.findFirst({
...
export const updateRestaurantVoucherStatus = async (
