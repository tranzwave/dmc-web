...
const NotificationModalTrigger: React.FC = () => {
    const { notifications, unreadCount, setNotifications, setUnreadCount, markAllAsRead, markAsRead } = useNotificationPolling();
    const [open, setOpen] = useState(false);
    const router = useRouter();

    const handleClick = (notification:SelectNotification) => {
        ...
        <Popover>
            ...
            <PopoverContent className="w-96 shadow-lg">
                <div className="flex justify-between items-center">
                    <h3 className="text-[15px] font-semibold">Latest Notifications</h3>
                    {unreadCount > 0 && (
                ...
                <div className="mt-2 flex flex-col gap-2 max-h-[550px] overflow-y-auto">
                    {notifications.length === 0 ? (
                        ...
                        notifications.map((notification, index) => (
                            <NotificationItem key={index} notification={notification} onClick={()=>handleClick(notification)} />
                        ))
                    )}
                </div>
            </PopoverContent>
